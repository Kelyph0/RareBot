...
class Moderation(commands.Cog):
    ...
    async def kick(self, ctx, member: discord.Member, *, reason=None):
        """
        Kicks a selected member.
        """
        await ctx.guild.kick(user=member, reason=reason)

        channel = self.bot.get_channel(720751131705475213)
    ...
    async def purge(self, ctx, amount=15):
        """
        Clears text chat, default is 15 but a select amount can be chosen by adding a number after the command.
        """
